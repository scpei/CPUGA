        ...
        with tf.variable_scope(self.name + '_vars'):
            for i in range(len(self.support)):
                ...
                self.vars['weights_' + str(i)] = init([input_dim, output_dim],
                                                        name='weights_' + str(i))
                self.vars['weights_2' + str(i)] = init([input_dim, output_dim],
        ...
        for i in range(len(self.support)):
            if 'weights_'+str(i) in self.vars:
                if not self.featureless:
                    ...
                    mes = tf.add(pre_sup, dot(tf.multiply(x, x), self.vars['weights_2' + str(i)], sparse=self.sparse_inputs))
                else:
                    pre_sup = self.vars['weights_' + str(i)]
                    mes = pre_sup
            else:
                mes = x
            support = dot(self.support[i], mes, sparse=True)
